        ...
        <div className="max-w-2xl mx-auto px-6 py-4 flex items-center justify-between text-white">
           ...
           <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-6">
              <div className="flex items-center space-x-2">
                 ...
                 <h2 className="font-serif text-base font-bold text-white uppercase tracking-widest shadow-sm">
                   The Path
                 </h2>
              </div>
           ...
           <div className="flex items-center space-x-3 pl-4">
              <span className="text-sm font-serif font-bold text-white">{progressPercentage}%</span>
              <div className="w-20 h-2 bg-black/20 rounded-full overflow-hidden backdrop-blur-sm">
            ...
            return (
              <div key={stage.id} className="relative">
                ...
                {stageIndex < MOKSHA_STAGES.length - 1 && (
                  <div className="absolute left-8 top-16 bottom-0 w-0.5 bg-stone-200 dark:bg-slate-800 -z-10"></div>
                )}
                ...
                <div 
                  ...
                  className={`
                    ...
                    ${isExpanded 
                      ? 'bg-stone-900 text-white border-stone-900 shadow-2xl scale-[1.02] dark:bg-slate-100 dark:text-slate-900' 
                      : isCurrentPhase
                        ? 'bg-white dark:bg-slate-900 border-saffron-400 dark:border-saffron-500 ring-2 ring-saffron-100 dark:ring-saffron-900/30 shadow-lg'
                ...
                >
                  <div className="p-5 flex items-center justify-between">
                    <div className="flex items-center space-x-5">
                      ...
                      <div className={`
                        w-14 h-14 rounded-2xl flex flex-col items-center justify-center font-serif font-bold text-xl shadow-inner transition-colors relative
                        ${isExpanded 
                      ...
                      `}>
                        <span className="text-[10px] font-sans font-normal uppercase opacity-80">Stage</span>
                        {stage.id}
                      ...
                      <div>
                        <div className="flex items-center space-x-2">
                           <h3 className={`font-serif text-lg font-bold tracking-wide ${isExpanded ? 'text-white dark:text-slate-900' : 'text-stone-900 dark:text-slate-100'}`}>
                             {stage.name}
                           </h3>
                           {isCurrentPhase && !isExpanded && (
                             <span className="px-2 py-0.5 bg-saffron-100 dark:bg-saffron-900/30 text-saffron-700 dark:text-saffron-300 text-[9px] font-bold uppercase tracking-widest rounded-full animate-pulse">
                               Current Phase
                             </span>
                           )}
                        </div>
                        ...
                        <div className="flex items-center mt-1 space-x-3">
                           <div className={`h-1.5 w-24 rounded-full overflow-hidden ${isExpanded ? 'bg-white/20 dark:bg-black/20' : 'bg-stone-200 dark:bg-slate-700'}`}>
                           ...
                           <span className={`text-[10px] font-bold uppercase tracking-widest ${isExpanded ? 'text-white/70 dark:text-slate-900/70' : 'text-stone-500 dark:text-slate-400'}`}>
                    ...
                    {!isStageLocked && (
                      <div className={`transform transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`}>
                        <ChevronDown size={20} className={isExpanded ? 'text-white dark:text-slate-900' : 'text-stone-400'} />
                      </div>
                    )}
                    {isStageLocked && <Lock size={20} className="text-stone-400" />}
                  </div>
                </div>

                {/* Lesson List */}
                <div className={`
                   mt-4 ml-6 pl-8 border-l-2 border-stone-200 dark:border-slate-800 space-y-4 transition-all duration-500 ease-in-out
                   ${isExpanded && stageLessons.length > 0 ? 'max-h-[2000px] opacity-100 pb-8' : 'max-h-0 opacity-0 overflow-hidden'}
                `}>
                  {stageLessons.length === 0 && isExpanded && (
                    <div className="p-4 text-sm text-stone-500 italic">Coming soon...</div>
                  )}
                  {stageLessons.map((lesson, idx) => {
                    ...
                    return (
                      <button 
                        ...
                        disabled={isLocked}
                        className={`
                          group relative w-full text-left rounded-xl border-2 transition-all duration-300 p-5 sm:p-6
                          ${isCompleted 
                            ? 'bg-emerald-50/50 dark:bg-emerald-900/10 border-emerald-100 dark:border-emerald-900/30' 
                            : isNext 
                              ? 'bg-saffron-500 dark:bg-saffron-600 border-saffron-500 dark:border-saffron-600 shadow-xl shadow-saffron-500/30 scale-[1.02] z-10' // PRIMARY BACKGROUND
                              : isLocked
                                ? 'bg-stone-50 dark:bg-slate-900 border-transparent opacity-60'
                                : 'bg-white dark:bg-slate-800 border-white dark:border-slate-700 hover:border-stone-200 dark:hover:border-slate-600'}
                        `}
                      >
                         {/* Timeline Node Connector */}
                        <div className={`
                          absolute -left-[41px] top-1/2 -translate-y-1/2 w-5 h-5 rounded-full border-2 z-10 transition-colors duration-300
                          ${isCompleted 
                            ? 'bg-emerald-500 border-emerald-500' 
                            : isNext 
                              ? 'bg-white border-saffron-500 ring-4 ring-saffron-200 dark:ring-saffron-900/50' 
                              : 'bg-stone-200 dark:bg-slate-700 border-stone-200 dark:border-slate-700'}
                        `}>
                           {isCompleted && <CheckCircle2 size={14} className="text-white absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" />}
                           {isNext && <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-2 h-2 bg-saffron-500 rounded-full animate-pulse"></div>}
                        </div>

                        {/* Horizontal Line Connector */}
                        <div className="absolute -left-8 top-1/2 w-6 h-0.5 bg-stone-200 dark:bg-slate-800"></div>

                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            <div className="flex items-center space-x-2 mb-2">
                               <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full ${isCompleted ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900 dark:text-emerald-300' : isNext ? 'bg-white/25 text-white backdrop-blur-sm' : 'bg-stone-100 text-stone-500 dark:bg-slate-700 dark:text-slate-400'}`}>
                                 Step {idx + 1}
                               </span>
                               {isNext && <span className="text-[10px] font-bold uppercase tracking-widest text-white flex items-center shadow-sm"><Sparkles size={10} className="mr-1"/> Recommended</span>}
                            </div>
                            
                            <h4 className={`font-serif text-xl font-bold leading-tight mb-2 ${isCompleted ? 'text-emerald-900 dark:text-emerald-100' : isNext ? 'text-white' : 'text-stone-900 dark:text-slate-100'}`}>
                              {lesson.title}
                            </h4>
                            
                            <p className={`text-sm line-clamp-2 leading-relaxed ${isNext ? 'text-white/90' : 'text-stone-500 dark:text-slate-400'}`}>
                          ...
                          <div className="pl-4 pt-2">
                             {isLocked ? (
                               <Lock size={18} className="text-stone-300 dark:text-slate-600" />
                             ) : isCompleted ? (
                               <div className="w-8 h-8 rounded-full bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center">
                                 <Star size={16} className="text-emerald-600 dark:text-emerald-400 fill-current" />
                               </div>
                             ) : (
                               <div className={`w-10 h-10 rounded-full transition-colors flex items-center justify-center ${isNext ? 'bg-white/20 text-white' : 'bg-stone-100 dark:bg-slate-700 text-stone-400 group-hover:bg-saffron-100 group-hover:text-saffron-600'}`}>
